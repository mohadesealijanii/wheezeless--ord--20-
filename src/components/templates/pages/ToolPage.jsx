...
import { deleteTool, filterTool } from "../../../utils/services"
...
import ToolModal from "../../modules/module/ToolModal"
function ToolPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        const res = await filterTool(paginator)
        setData(res.data)
        console.log(res)
        setLoading(false)
      } catch (error) {
  ...
  const categoryRow = (item) => (
    <tr className="border-b border-dark/10 h-15 text-center">
      ...
      <ToolTip id={`parameter-${item.id}`} content="Tool Parameters" />
  ...
  const addButton = () => (
    ...
    >
      ...
      <span className="pr-1">Add Tool</span>
    </button>
  )
  ...
  return (
    <>
      ...
      {modal.add && (
        <ToolModal setRefreshTrigger={setRefreshTrigger} setModal={setModal} />
      )}
      {modal.delete && (
        <ReusableDeleteModal
          setRefreshTrigger={setRefreshTrigger}
          ...
          loading={loading}
          headerText={`"${selectedItem.name}" tool`}
        />
      )}
      <Table
        title="Tools List"
