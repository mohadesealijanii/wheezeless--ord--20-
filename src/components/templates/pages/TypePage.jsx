...
import { deleteType, filterType } from "../../../utils/services"
...
import TypeModal from "../../modules/module/type/TypeModal"
function TypePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        const res = await filterType(paginator)
        setData(res.data)
        console.log(res)
        setLoading(false)
      } catch (error) {
  ...
  const addButton = () => (
    ...
    >
      ...
      <span className="pr-1">Create a Type</span>
    </button>
  )
    ...
    <>
      ...
      {modal.delete && (
        <ReusableDeleteModal
          ...
          loading={loading}
          headerText={`"${selectedItem.name}" type`}
        />
      )}
      <Table
        title="Type List"
