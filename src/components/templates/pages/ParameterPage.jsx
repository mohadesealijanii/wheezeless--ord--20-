...
import { deleteParameter, filterParameter } from "../../../utils/services"
...
import ParameterModal from "../../modules/module/parameter/ParameterModal"
function ParameterPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        const res = await filterParameter(paginator)
        setData(res.data)
        console.log(res)
        setLoading(false)
      } catch (error) {
  ...
  const addButton = () => (
    ...
    >
      ...
      <span className="pr-1">Create a Parameter</span>
    </button>
  )
    ...
    <>
      ...
      {modal.delete && (
        <ReusableDeleteModal
          ...
          loading={loading}
          headerText={`"${selectedItem.name}" Parameter`}
        />
      )}
      <Table
